...
var NelioABGoalCards = {
	...
	createCard: function( id ) {
		...
		newCard.find('.new-actions .phone-call').click(function() {
			NelioABGoalCards.addAction( { isNew:true, type:'phone-call' }, newCard );
		});
	...
	addAction: function(action, card) {
		...
		switch ( action.type ) {
			...
			case 'phone-call':
				if ( card.find( '.actions .action.phone-call' ).length < 1 ) {
					result = jQuery('#new-action-templates').find( '.action.phone-call' ).clone();
					card.find( '.actions' ).append( result );
					card.find( '.empty' ).hide();
					card.find( '.actions' ).show();
					result.show();
