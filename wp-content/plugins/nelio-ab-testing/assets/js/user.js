...
NelioAB.user.loadEnvironment = function( params ) {
	...
	if ( abForThisPage ) {
		...
		if ( NelioABEnv ) {
			...
			// If it is a new environment, I'll have update the cookie
			if ( isNewEnv ) {
 ...
 *               A/B testing.
 */
NelioAB.user.loadEnvironmentAndSetAlternatives = function( params, overwriteCookies ) {
	...
	if ( params.length > 0 && overwriteCookies ) {
		var abForThisPage = false;
		var globalVals = [];
