...
NelioAB.checker.init = function() {
	// Let's start by generating a user ID, no matter what.
	NelioAB.user.id();
...
NelioAB.checker.assignAlternativesAndCheck = function() {
	...
	aux = [];
	var overwriteCookies = false;
	for ( var i = 0; i < getParams.length; ++i ) {
		if ( NelioAB.checker.nabMatcher.test( getParams[i][0] ) ) {
			aux.push( getParams[i] );
		}
		if ( getParams[i][0] === 'nabforce' ) {
			overwriteCookies = true;
		}
	}
	NelioAB.user.loadEnvironmentAndSetAlternatives( aux, overwriteCookies );
	...
	// (c) there was the param named nabforce
	if ( overwriteCookies ) {
		areThereTooManyParams = true;
	}
