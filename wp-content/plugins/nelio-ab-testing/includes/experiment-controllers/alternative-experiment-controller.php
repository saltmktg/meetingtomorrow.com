...
class NelioABAlternativeExperimentController {
	...
	public function hook_to_wordpress() {
		...
		add_action( 'wp_head', array( &$this, 'add_disqus_fix' ), 1 );
	...
	public function fix_link_with_experiments( $permalink ) {
		...
		// Do not change the canonical link for the front page.
		if ( get_option( 'show_on_front' ) == 'page' &&
				untrailingslashit( get_option( 'home' ) ) == untrailingslashit( $permalink ) ) {
		...
		if ( $post_id == $current_id ) {
			...
			foreach ( array( 'nabc', 'nabm', 'nabt', 'nabw' ) as $type ) {
				if ( $links_info[$type] ) {
					$permalink = add_query_arg( $type, $links_info[$type], $permalink );
				}
			}
	...
	public function load_comments_number_from_original( $comments_number ) {
		...
		if ( get_post_meta( $id, '_nelioab_hide_discussion', true ) ) {
			return 0;
		}
	 ...
	 * @since 4.3.1
	 */
	public function add_disqus_fix() {
		if ( is_single() || is_page() ) {
			global $nelioab_controller;
			$post_id = $nelioab_controller->get_queried_post_id();
			if ( $this->is_post_alternative( $post_id ) ) {
				$post_id = $this->get_original_related_to( $post_id );
			}
			$url = get_permalink( $post_id );

			/**
			 * Filter whether a page ID has to be included in disqus' config or not.
			 *
			 * @param boolean $include_identifier Whether a page ID has to be included in disqus' config or not.
			 *
			 * @since 4.3.1
			 */

			$include_identifier = apply_filters( 'nelioab_use_page_id_in_disqus', false );
			?>
			<script type="text/javascript">
				var disqus_config = function() {
					...
					if ( $include_identifier ) { ?>
						this.page.identifier = '<?php echo $post_id; ?>';
					<?php
					} ?>
				};
			</script>
