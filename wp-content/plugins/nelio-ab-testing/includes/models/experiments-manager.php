	...
	class NelioABExperimentsManager implements iNelioABDataManager {
		...
		public static function update_running_experiments_cache( $force_update = 'if-required' ) {
			...
			if ( self::CACHE_ALL_EXPERIMENTS ) {
				...
				try {
					$old_running_experiments = get_option( 'nelioab_running_experiments', array() );
					...
					foreach ( $experiments as $exp_short ) {
						...
						/** @var NelioABExperiment|boolean $exp */
						$exp = false;

						/** @var NelioABExperiment $candidate */
						foreach ( $old_running_experiments as $candidate ) {
							if ( $candidate->get_id() === $exp_short->get_id() ) {
								$exp = $candidate;
								break;
							}
						}
					...
					// UPDATE TIMESTAMP
					$last_start_or_stop = get_option( 'nelioab_last_start_or_stop', 0 );
					foreach ( $experiments as $exp ) {

						$val = $exp->get_start_date();
						if ( ! empty( $val ) ) {
							$val = strtotime( $val );
							if ( $last_start_or_stop < $val ) {
								$last_start_or_stop = $val;
							}//end if
						}//end if

						$val = $exp->get_end_date();
						if ( ! empty( $val ) ) {
							$val = strtotime( $val );
							if ( $last_start_or_stop < $val ) {
								$last_start_or_stop = $val;
					...
					update_option( 'nelioab_last_start_or_stop', $last_start_or_stop );
