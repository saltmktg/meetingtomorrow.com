	...
	abstract class NelioABAltExpProgressPage extends NelioABAdminAjaxPage {
		...
		protected function print_actions_info() {
			...
			if ( count( $phone_call_actions )>0 )
				$this->print_phone_call_actions_box( $phone_call_actions );
		}
		...
		private function select_phone_call_actions( $action ) {
...
HTML;
		...
		protected function print_phone_call_actions_box( $actions ) {
			$this->print_beautiful_box(
				'nelio-phone-call-actions',
				$this->get_action_heading( NelioABAction::PHONE_CALL ),
				array( &$this, 'print_phone_call_actions_content', array( $actions ) ) );
		}

		protected function print_phone_call_actions_content( $actions ) {

			foreach ( $actions as $action ) {

				$message = __( 'A visitor calls to your ResponseTap account.', 'nelioab' );
		...
		protected function get_action_heading( $type ) {
			switch ( $type ) {
				...
				case NelioABAction::PHONE_CALL:
					...
					$alt = __( 'A visitor calls to your ResponseTap account.', 'nelioab' );
					$title = __( 'Phone Call', 'nelioab' );
					break;
			}
...
HTML;
		...
		protected function print_experiment_information( $exp, $descr, $res ) { ?>
			...
			$duration_title = __( 'Elapsed Time', 'nelioab' );
			if ( $res == null && $exp->get_status() == NelioABExperiment::STATUS_FINISHED ) {
				$duration_title = __( 'Duration', 'nelioab' );
			...
			<div id="exp-info-running-time">
				<span class="exp-info-header"><?php echo $duration_title; ?></span>
