...
if ( !class_exists( 'NelioABWooCommerceSupport' ) ) {
	...
	class NelioABWooCommerceSupport {
		 ...
		 *
		 * @param  int     $order_id    PHPDOC
		 * @param  string  $old_status  PHPDOC
		 * @param  string  $new_status  PHPDOC
		 *
		 * @return void
		 *
		 * @since PHPDOC
		...
		public function sync_order( $order_id, $old_status, $new_status ) {
			...
			$expected_statuses = apply_filters( 'nelioab_order_status_for_conversions', 'completed' );
			if ( ! is_array( $expected_statuses ) ) {
				$expected_statuses = array( $expected_statuses );
			}

			if ( ! in_array( $new_status, $expected_statuses ) ) {
				return;
			}
